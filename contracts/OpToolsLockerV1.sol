...
contract OpToolsLockerV1 is AccessControl, ReentrancyGuard {
    ...
    function relockToken(
        ...
        mustBeEnabled 
        mustBeOwner(lockId) 
        mustBeUnlocked(lockId) 
    {
        require (amount > 0, "Amount must be larger than zero");
        require (amount <= locks[lockId].amount.sub(locks[lockId].withdrawnAmount), "Amount is bigger than eligible amount for relock");
        require (timestamp >= block.timestamp.add(24 hours), "Unlock timestamp must be at least 24 hours in the future");
        require (msg.value >= lockFee, "Payable amount must be at least equal to lock fee");
        address token = locks[lockId].token;
        ...
        LockInfo memory lockInfo = LockInfo(true, token, owner, amount, timestamp, 0);
        locks[currentLockId] = lockInfo;
        emit TokenLocked(currentLockId, token, owner, amount, timestamp);
        currentLockId++;
    }
        ...
        mustBeUnlocked(lockId) 
    {
        ...
        IERC20(token).safeTransfer(locks[lockId].owner, amount);
        emit TokenUnlocked(lockId, amount);
    }

    /** INTERNAL FUNCTIONS */

    function _removeLockIdFromUserArray(
        uint lockId, 
        address owner
    ) 
        internal 
